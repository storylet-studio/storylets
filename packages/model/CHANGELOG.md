...
- 3a195ca: `PlayRecord.outcome` documents the empty outcome: `""` for a card with no outcomes, played with none (the no-outcome-play brief, 2026-09-14). The key is always present, so a save's shape does not depend on the card.
...
- 73db45c: Hand positions in the bundle's `maps` block (design/engine-server.md 4.3).
...
- 73db45c: The play ladder: one project setting that decides how much of the editor you see (design/engine-server.md 4.10).
...
- 73db45c: A box that counts in time: `turn: { seconds: N }` (design/engine-server.md 4.8).
...
- 73db45c: The installation contract: what an author may not break (design/engine-server.md 4.11, net 1).
...
- 73db45c: Park one flow, and price a load before you take it (design/engine-server.md 4.1 and 4.9).
...
- c561a76: `valueAddresses(bundle)`: the owner segment of a tag's property address, box-qualified where it has to be (design/engine-server.md 4.4).
...
- 73db45c: The durability axis: `durable` on a declaration, a deck and a card (design/engine-server.md 4.2).
...
- 73db45c: A hole filled from a property: the hand that moves (design/engine-server.md 4.6).
...
- 4ad8933: `LoadProperty.path` is documented as the gameId form (design/engine-server.md 4.4).
