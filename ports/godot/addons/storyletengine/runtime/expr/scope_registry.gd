# GENERATED - vendored from expr/ports/godot/scope_registry.gd by scripts/vendor-ports.mjs.
...
var revision: int:
	get:
		return _rev
	set(_value):
		push_error("StoryletScopeRegistry: revision is read-only")
...
func to_eval_context(host = null, opts: Dictionary = {}) -> Dictionary:
	...
	if view.has("error"):
		push_error("StoryletScopeRegistry: " + str(view["error"]))
