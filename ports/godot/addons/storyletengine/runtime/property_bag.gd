...
# @wildwinter/scoperegistry's PropertyBag (the properties implementer Patter
# will share). A typed, declared property bag with defaults, the firing rule
...
# ignored). Values are StoryletValues scalars (bool / float / String / Array
# of String).
#
# Name normalisation policy: lowercase by default (the registry's
# long-standing contract); storylets property names are case-significant as
# authored, so the session passes identity.
...
func _seed(declarations: Array) -> void:
	for d in declarations:
		...
		values[name] = _copy_value(StoryletValues.to_value(d.get("default", default_for(d))))
...
func set_value(name: String, value, opts: Dictionary = {}) -> Dictionary:
	...
	if _decls.has(n) and _decls[n].get("writable", true) == false:
		...
		push_error("StoryletPropertyBag: " + msg)
		return {"error": msg}
	var change := {
		"name": n,
		"next": value,
...
func rows() -> Array:
	...
	for name in _decls:
		...
		var row := {
			...
			"default": _copy_value(StoryletValues.to_value(d.get("default", default_for(d)))),
...
func clone() -> StoryletPropertyBag:
	var c := StoryletPropertyBag.new([], {"normalise": _normalise})
...
func load(saved: Dictionary) -> void:
	for k in saved:
		values[_normalise.call(str(k))] = _copy_value(StoryletValues.to_value(saved[k]))
