...
- `StoryletLiveLink.apply_live_bundle` refuses a swap for an engine that was given the game's registry, as the JS runtime does: the new engine's tokens would clash with the old one's there. The old engine is left untouched.
