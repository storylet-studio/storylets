...
namespace StoryletStudio.StoryletEngine
{
    ...
    public sealed class PropertyBag
    {
        ...
        public BagChange Set(string name, StoryletValue value, bool silent = false, string reason = null)
        {
            var n = _norm(name);
            var decl = _decls.GetOrDefault(n);
            if (decl != null && decl.Writable == false) throw new StoryletError($"'{name}' is read-only");
