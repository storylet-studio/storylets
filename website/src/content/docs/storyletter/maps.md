---
title: Maps
description: Turn a tag group into a map, with zones you draw, hands pinned inside them, and a drag that rebinds a hand to a new zone.
sidebar:
  label: Maps
---

A tag group can be a place. Turn that on and its tags stop being a list of names and
become zones you draw, outlines on a canvas with hands pinned to positions inside them. Drag a hand from one zone to another and it's rebound to the new zone's tag.
...
One thing to hold on to is that **a map is a drawing of tags you already have**. There's no
geometry in the engine, and nothing in a bundle that a runtime reads as a shape. A zone is
a tag, and a site is where you chose to draw a hand. Delete the whole map and the project
plays identically.
...
There are two routes to the same thing, because a map **is** a tag group and both make one.
**Maps tab ▸ + New map** works before you have any, because the box always has a Maps tab.
It creates the group and marks it a map in one step. **Tags tab ▸ + New map**, beside
**+ New tag group**, is there if you are already on that tab.
...
Maps tab, which then holds an invitation rather than a canvas.
...
under it has been moved off the zone it's bound to. **A hollow site** is a hand nothing is
binding. It marks a spot and no more.
...
it there and moves it the first time you touch the map, and `storyletengine format` moves
