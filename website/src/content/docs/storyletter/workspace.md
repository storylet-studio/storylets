---
title: The workspace
description: Find your way around Storyletter's navigator, document tabs, problems bar, Find window, menus, project settings, and themes.
sidebar:
  label: The workspace
---

Storyletter has two panes, the navigator down the left and the document you're editing in
...
The **chevron** expands or collapses a row and never navigates, and clicking a **label**
opens that document. Only chevron clicks are remembered. The path to whatever you have open
expands while it's open, so the tree doesn't ratchet itself open over a week of work. The
open document's row is highlighted strongly, and each ancestor softly. When the open
...
condition it is. On a card it reads "the condition to be dealt", on a deck "the condition
for any card in this deck", and on an outcome "the condition for this outcome to be
offered".

Conditions and outcome changes are edited with a guided expression editor rather than free
...
When something is wrong, a one-line bar appears along the bottom, with the count and one
...
a row to go there. The [Coverage](/production/coverage-testing/) window's "gated on `@x`"
...
own](/storyletter/overview/#a-playable-page-one-file-plays-anywhere). **Publish
...
only about what you're shown. Nothing else is governed by it. A timed box and a hole
...
editor honours that rung, shows it in the field, and lets you move down from it; it isn't
one you can pick.
...
its type. Boolean and enum values are pickers rather than free text.
...
There are four palettes under **View ▸ Colour Theme**, plus **Follow System**. **Chambray**
(light) and **Indigo** (dark) are the defaults, in blue-grey. **Linen** and **Baize** are
their green-tinted predecessors, kept for anyone who prefers them. Follow System switches
between Chambray and Indigo with your OS. Every window and dialog follows the theme,
including the Board, Find, and Coverage, and switching between same-lightness palettes
never recolours your tags, decks, or canvas furniture. Those colours are your content's,
not the theme's.

## Links

**Review ▸ Links…** opens a lens on the card you have open. What can turn it on or off sits
to the left, and what it turns on or off to the right, across every deck and box. It follows
the editor, so it can sit open beside you. [The Links window](/storyletter/links/) has the
detail.
