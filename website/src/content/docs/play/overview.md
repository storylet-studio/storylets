...
description: Load a compiled bundle in JavaScript, Unity, Unreal, or Godot and deal your first hand with the Storylet Engine runtime for your engine.
sidebar:
  label: Overview
---

Your designers' cards reach the player through a Storylet Engine runtime. You publish the
project to one `.storyletsc` [bundle](/format/bundle/), drop it into your engine, and the
...
There's one runtime per engine, for JavaScript, Unity, Unreal, and Godot. Every one is
checked against the same [test suite](/compatibility/), so the same bundle deals the same
cards everywhere, right down to the random draws. All four ship today.
...
5. Ask for the outcomes, offer the available ones, and play the one the player picks. State
   ...
   Pass `""` as the outcome once it has been shown.
...
Before you deal, there's one thing to wire up. **`@world`** is the set of properties your game
...
runtime. [Compatibility & conformance](/compatibility/) says how that's checked.
...
carries a [Live Link](/play/live-link/) client. Connect your running game to Storyletter, and
saves reach the run without a restart while the Board shows the game's deals as they happen.
