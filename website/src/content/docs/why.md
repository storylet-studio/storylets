...
what the tool is for, what it gives you that a pile of `if` statements doesn't, and where it
sits beside the tools you've probably already got.

Storylet Studio is indie-first, but team-ready. Your content is plain files you own, your
designers work in an editor that needs no code and no wiring, and one small compiled file
plays in the engine you ship on (JavaScript, Unity, Unreal, or Godot) exactly as it played in
the editor. It leans on your existing version control rather than a server, and because the
format is built around the merge, it scales from one designer to a team on git or Perforce.
...
Wire that as branches and you get a complicated tree that spirals and is hard to debug and
expand. Wire it as `if` statements in your game code and the designers can't touch it. A
storylet system is the third answer. The beats are data with attached conditions, the
question is one call, and the answer comes back ranked.

## What you'd decide with it

Different games need different features, and storylets can help in different places. Each of
...
If your version of the question isn't listed, check for the same shape. A pool of
...
Storyletter ships this list made concrete as **Port Meridian**, a mini action-game demo
...
Ask what could happen here, and you get back the beats that fit, best first. It's one
question rather than a tree. Adding a new beat means designing one more card and saying when
it applies. You never rewire anything.

A deck is a pool, not a running order, so where a card sits in the file means nothing. Two
designers can add cards to the same deck at the same time without their work colliding, which
is the thing that usually makes shared story files painful.

Nothing repeats unless you say so. There's one copy of every card, so the same rumour can't
be offered in two places at once, and a card that's been played stays away for as long as
you said. You get that without setting a flag.

The order is one you can predict. The most important card first, then whichever one's
conditions are the most specific, then chance. Start from the same seed and you get the same
run every time, in every engine.

"Why didn't that happen?" has an answer. Every time you ask, the engine records what it
...
Dead content is found before players find it.
[Coverage testing](/production/coverage-testing/) plays the project hundreds of times and
lists every card nothing ever reached and every outcome nobody ever took. It runs from the
command line too, so you can fail a build on it.

The files are yours, in your version control. A project is a folder of plain text files, one
...
changes. The presentation of your beat belongs to your game, or to Patter. So there's no
translation system here to work around and no second list of strings to keep in step.
...
If you're writing a **conversation**, a scene with lines, choices, and a shape, that's a
branching tool's job, and [Patter](https://patterkit.dev), Ink, or Yarn Spinner will feel
right.

If your beats carry **the words themselves** and you need them localised and voiced, remember
that cards hold no text. Pair this with [Patter](https://patterkit.dev), or keep the text in
your own system and put its key on the card.

If you need live, Google-Docs-style **co-editing** of one deck, this isn't it. Storylet Studio
leans on your version control (with a structured merge that combines edits rather than
colliding), which scales to a big team but is check-out-and-merge, not simultaneous.

## Where to start

[Getting started](/getting-started/) is the path from download to a hand dealt in your game,
and [Core concepts](/concepts/) is the vocabulary on one page. If you're evaluating for a
team, skim [Playing in your game](/play/overview/) for the integration story on your engine,
and [Version control](/setup/version-control/) for how a team shares a project.
