...
description: See how a .storylets project is a folder of small JSON5 files, always written the same way and sorted by id, so a team's edits combine instead of colliding.
sidebar:
  label: Overview
---
...
pack, the round trip is self-contained, so neither end needs access to the other's version
control.
